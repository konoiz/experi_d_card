...
(function() {
  $(document).on('ready page:load', function() {
    ...
    /* 郵便番号入力フォーム：数値のみの入力 */
    $('#card_postalcode').keydown(function() {
      var presskey;
    ...
    $('#getarea-button').click(function() {
      ...
      return $.ajax({
        ...
        type: "GET",
        data: {
          search_code: postalcode
        },
        dataType: "json",
        context: this,
        error: function(jqXHR, textStatus, errorThrown) {
          return $("#postalcode-result").css("color", "#ff0000").html(errorThrown);
        },
        success: function(data, textStatus, jqXHR) {
          if (data != null) {
            $("#card_address_prefectural").val(data.prefectural_id);
            $("#card_address_city").val(data.city);
            $("#card_address_street").val(data.street);
            return $("#postalcode-result").css("color", "#00dd00").html("入力が完了しました。");
          } else {
            return $("#postalcode-result").css("color", "#ff0000").html("未登録の郵便番号です。");
          }
        }
      });
    });
    ...
    return $('.address').change(function() {
      return $("#postalcode-result").html("");
    });
  });

  $(function() {
    return $(document).on('change', '#card_department', function(evt) {
      $.ajax('update_courses', {
        type: 'GET',
        dataType: 'script',
        data: {
        ...
        success: function(data, textStatus, jqXHR) {
          return console.log("45 ok");
        }
      });
      return $.ajax('update_laboratories', {
        type: 'GET',
        dataType: 'script',
        data: {
          department: $("#card_department option:selected").val()
